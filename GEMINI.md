...
8. **Multi-Agent Review Panel & Tiered Engineering Workflow**:
   - During self-improvement workflows, code review is structured dynamically across four complexity tiers:
     ...
     - **Tier 2 (50–200 LOC, standard features & multi-package refactors)**: Full 4-expert review panel (3 Domain Specialists + 1 Devil's Advocate) audits the plan inline within the root thread across the 4 specialized disciplines (~45s). Autonomous execution (no mandatory stop; directly incorporates plan feedback). Zero mid-task pauses during coding. Consolidated Devil's Advocate audits the unified `git diff` inline before opening the PR (~30s).
     - **Tier 3 (> 200 LOC, core architecture, database migrations, breaking changes)**: Full 4-expert review panel audits the plan inline within the root thread across the 4 specialized disciplines. **MANDATORY HUMAN REVIEW CHECKPOINT (STOP)**: synthesize panel findings and wait for explicit user approval before touching code. Continuous implementation once approved (zero mid-task pauses). Full 4-expert review panel audits the unified `git diff` inline before merge (~45s).
   - **Continuous Autonomous Execution & Review Panel Timing**:
     ...
     - **Inline Review Panel Execution (Zero Subagent Gathering Hazard)**: In headless Discord bot turns (`agy -p`), review panels (The Girl Gang) MUST execute inline within the root thread across the 4 specialized disciplines (Systems & Concurrency, Domain Specialist, PWA/Frontend, Adversarial Devil's Advocate). Dispatching review panels to background subagents and attempting to hold `agy` open via `schedule` is strictly prohibited—`agy`'s print-mode drain kills background tasks upon conversational yield and leaks intermediate waiting notices to Discord. Inline evaluation guarantees atomic, zero-drain synthesis directly delivered to Discord.
