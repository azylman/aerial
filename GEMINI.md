...
8. **Multi-Agent Review Panel & Tiered Engineering Workflow**:
   ...
   - **Continuous Autonomous Execution & Review Panel Timing**:
     ...
     - **Orchestration & Subagent Delegation Invariant**: For Tier 1+ multi-file edits, test triage, and lint remediation loops, the primary agent acts as an orchestrator, delegating heavy tool loops to scoped, isolated subagents (aligned with Section 5 lines 127–128 and the `subagent-driven-development` skill). Tier 0 micro-changes (≤ 5 LOC) execute directly in the root thread. Root transcript must remain featherweight (<50 steps) to prevent quadratic token compounding.
