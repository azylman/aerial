...
- **Automated Two-Phase PR Workflow (`aerial-config-pr.sh`)**:
  ...
  3. `aerial-config-pr.sh submit <scratch_dir> "<commit message>"`: Performs pre-flight YAML validation, validates the mandatory PR description, pushes the branch, opens a Pull Request on GitHub, automatically schedules a one-shot follow-up reminder via `scheduler-mcp` (defaulting to 2m), and discards the scratch clone.
...
8. **Multi-Agent Review Panel & Tiered Engineering Workflow**:
   - During self-improvement workflows, code review is structured dynamically across four complexity tiers:
     ...
     - **Tier 1 (< 50 LOC, targeted bugfixes & test additions)**: Solo Adversarial Systems Critic / Devil's Advocate audits the plan (~30s). Autonomous execution (no mandatory stop). Zero mid-task pauses during coding. Direct local pre-flight verification.
