...
6. **Continuous Deployment & Engineering Invariant**:
   ...
   - **Exclusively Asynchronous PR Submission & Automated Follow-Up (`aerial-pr.sh submit`)**: `scripts/aerial-pr.sh submit` and `scripts/aerial-config-pr.sh submit` operate strictly asynchronously. Synchronous submission mode has been eliminated. Fast pre-flight verification (`scripts/verify.sh --staged`) runs locally in <1s, pushes the branch, and creates the PR immediately. A detached background daemon monitors CI and automatically squash-merges upon success, allowing the Discord response to be delivered immediately without blocking on long-running GitHub Actions builds. The script automatically schedules a one-shot follow-up check via `scheduler-mcp` (defaulting to 13m for `aerial` and 2m for `aerial-config`), automatically resolving the active thread via `AERIAL_TARGET_ID` (injected by the execution runner) or explicit `--target-id <thread_or_channel_id>`. Aerial MUST NOT manually schedule an extra follow-up reminder after calling `submit`. When the scheduled check wakes up, Aerial inspects the PR status, confirms the deployment, or self-heals by completing the squash-merge if the background daemon terminated.
   - **Automated PR Descriptions & Title Hygiene**: `submit` supports a 4-tier resolution hierarchy for pull request descriptions:
     1. Explicit flag: `--body <body>` / `-b <body>` or `--body-file <path>` / `-f <path>`.
     2. Workspace convention file: authoring `PR_DESCRIPTION.md` or `.pr_description.md` in the scratch root. The file is consumed and automatically deleted before staging so it is never committed to `main`.
     3. Multi-line commit message splitting: Line 1 becomes the single-line PR title, and subsequent lines become the PR description.
     4. Default automated repository placeholder.
