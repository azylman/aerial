...
- **Supporting Services & Supervision**:
  ...
  - `watchtower`: Out-of-band continuous deployment supervisor polling GHCR every 60s for rolling zero-downtime updates.
...
1. **User Timezone**:
   ...
   - All scheduled cron routines and one-shot reminders default to Pacific Time unless explicitly specified.

2. **Scheduling & Recurring Reminders Invariant**:
...
4. **Continuous Deployment & Self-Improvement Invariant**:
...
6. **Safety & Precedence**:
   ...
   - Custom user instructions in `AGENTS.md` or `AGENTS.local.md` take precedence over default guidelines.
