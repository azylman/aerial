...
func TestSyncComposeOverride_CreateAndCleanup(t *testing.T) {
	baseDir := t.TempDir()
	configDir := filepath.Join(baseDir, "aerial-config")
	projectDir := filepath.Join(baseDir, "aerial")

	if err := os.MkdirAll(configDir, 0755); err != nil {
		t.Fatalf("failed to create configDir: %v", err)
	}

	// 1. Initially no source file exists -> sync should succeed and do nothing
	if err := SyncComposeOverride(configDir, projectDir); err != nil {
		t.Fatalf("SyncComposeOverride failed on empty configDir: %v", err)
	}
	targetFile := filepath.Join(projectDir, "docker-compose.override.yml")
	if _, err := os.Lstat(targetFile); err == nil {
		t.Fatalf("Expected targetFile to not exist initially")
	}

	// 2. Create docker-compose.override.yml in configDir
	sourceFile := filepath.Join(configDir, "docker-compose.override.yml")
	composeContent := "services:\n  custom:\n    image: custom:latest\n"
	if err := os.WriteFile(sourceFile, []byte(composeContent), 0644); err != nil {
		t.Fatalf("failed to write source compose override: %v", err)
	}

	// 3. SyncComposeOverride creates symlink in projectDir
	if err := SyncComposeOverride(configDir, projectDir); err != nil {
		t.Fatalf("SyncComposeOverride failed to create symlink: %v", err)
	}

	if _, err := os.Lstat(targetFile); err != nil {
		t.Fatalf("targetFile does not exist: %v", err)
	}
	data, err := os.ReadFile(targetFile)
	if err != nil || string(data) != composeContent {
		t.Fatalf("targetFile content mismatch: %s, %v", string(data), err)
	}

	// 4. Idempotent rerun should succeed
	if err := SyncComposeOverride(configDir, projectDir); err != nil {
		t.Fatalf("Idempotent SyncComposeOverride failed: %v", err)
	}

	// 5. Replace target with dead symlink or regular file and verify resynchronization
	_ = os.Remove(targetFile)
	if err := os.WriteFile(targetFile, []byte("stale file"), 0644); err != nil {
		t.Fatalf("failed to write stale file: %v", err)
	}
	if err := SyncComposeOverride(configDir, projectDir); err != nil {
		t.Fatalf("SyncComposeOverride failed to overwrite stale file: %v", err)
	}
	data, err = os.ReadFile(targetFile)
	if err != nil || string(data) != composeContent {
		t.Fatalf("targetFile content mismatch after resync: %s, %v", string(data), err)
	}

	// 6. Delete sourceFile in configDir -> SyncComposeOverride cleans up obsolete symlink
	if err := os.Remove(sourceFile); err != nil {
		t.Fatalf("failed to remove sourceFile: %v", err)
	}
	if err := SyncComposeOverride(configDir, projectDir); err != nil {
		t.Fatalf("SyncComposeOverride failed during cleanup: %v", err)
	}
	if _, err := os.Lstat(targetFile); err == nil {
		t.Fatalf("Expected targetFile to be removed after source was deleted")
	}
}

func TestSyncComposeOverride_YamlExtensionFallback(t *testing.T) {
	baseDir := t.TempDir()
	configDir := filepath.Join(baseDir, "aerial-config")
	projectDir := filepath.Join(baseDir, "aerial")
	...
	// Create docker-compose.override.yaml (.yaml instead of .yml)
	sourceYaml := filepath.Join(configDir, "docker-compose.override.yaml")
	composeContent := "services:\n  yaml-ext:\n    image: yaml:latest\n"
	if err := os.WriteFile(sourceYaml, []byte(composeContent), 0644); err != nil {
		t.Fatalf("failed to write source yaml: %v", err)
	}

	if err := SyncComposeOverride(configDir, projectDir); err != nil {
		t.Fatalf("SyncComposeOverride failed with .yaml source: %v", err)
	}

	targetFile := filepath.Join(projectDir, "docker-compose.override.yml")
	data, err := os.ReadFile(targetFile)
	if err != nil || string(data) != composeContent {
		t.Fatalf("targetFile content mismatch with .yaml source: %s, %v", string(data), err)
	}

	// Cleanup on removal of .yaml
	_ = os.Remove(sourceYaml)
	if err := SyncComposeOverride(configDir, projectDir); err != nil {
		t.Fatalf("SyncComposeOverride cleanup failed for .yaml source: %v", err)
	}
	if _, err := os.Lstat(targetFile); err == nil {
		t.Fatalf("Expected targetFile to be removed after .yaml source was deleted")
	}
}

func TestSyncComposeOverride_EmptyInputs(t *testing.T) {
	if err := SyncComposeOverride("", "/project"); err != nil {
		t.Errorf("Expected nil error for empty configDir, got %v", err)
	}
	if err := SyncComposeOverride("/config", ""); err != nil {
		t.Errorf("Expected nil error for empty projectDir, got %v", err)
	}
}
